"""Plot the curriculum run: medium and hard eval curves with stage marks.
...
curriculum stage boundaries as vertical marks.

Run:  python scripts/plot_curriculum_eval.py [--out assets/curriculum_training_curve.png]
"""
...
STAGES = [(0.0, "demo x24"), (2.0, "medium 16 + demo 8"), (5.5, "hard 16 + medium 8")]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", default=str(ROOT / "assets" / "curriculum_training_curve.png"))
    args = ap.parse_args()

    data = {}
    for name, _ in SERIES:
        d = np.load(ROOT / "logs" / f"eval_pixels_{name}" / "evaluations.npz")
    ...
    fig.suptitle("Curriculum (v1.6): from the v2 easy-trained weights to medium and hard",
